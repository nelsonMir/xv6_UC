...
# ============================================
#  RAMDISK embebido: **fs.img → C (blob) → .o**
#  A PRUEBA DE BALAS: definiciones reales (no headers con definiciones)
# ============================================

# Generar un .c con las DEFINICIONES de fs_img y fs_img_len
kernel/fsimg_blob.c: fs.img
	@echo "Generando kernel/fsimg_blob.c desde fs.img..."
	@echo "/* AUTOGENERATED: do not edit */" > $@
	@xxd -i fs.img | sed 's/unsigned int fs_img_len;/unsigned int fs_img_len = sizeof(fs_img);/' >> $@

# Compilar el blob del fs embebido
$(K)/fsimg_blob.o: kernel/fsimg_blob.c
	$(CC) $(CFLAGS) -c -o $@ $<
